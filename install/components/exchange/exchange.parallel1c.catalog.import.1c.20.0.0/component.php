...
// Exchange++
//CIBlockCMLImport replace to CExchangeParallelsIBlockCMLImport
//$arParams["EXCHANGE_PARALLEL1C_SETTINGS_ID"] = intval($arParams["EXCHANGE_PARALLEL1C_SETTINGS_ID"]);

\Bitrix\Main\Loader::includeModule("exchange.parallel1c");
$arParams["EXCHANGE_PARALLEL1C_SETTINGS"] = \Exchange\Parallel1c\Tools::getSettingsByPath();

$table_name = "b_xml_tree_exchange_parallel1c_".$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["CODE"];
$catalog_name = "1c_catalog_exchange_parallel1c_".$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["CODE"];
// Exchange--
...
// Exchange++
elseif ( !$arParams["EXCHANGE_PARALLEL1C_SETTINGS"] )
{
	echo "failure\n";
	echo "exchange.parallels1c - settings not found\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/exchange_parallel1c_exchange_admin.php?lang=ru";
}
elseif
(
	$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["DISALLOW_IMPORT_XML_STEP"] == "Y"
	&&
	( $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import') )
)
{
	echo "success\n";
	echo "exchange.parallels1c - import.xml skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/exchange_parallel1c_exchange_admin.php?lang=ru";
}
elseif
(
	$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["DISALLOW_OFFERS_XML_STEP"] == "Y"
	&&
	( $_GET['mode'] == 'import' && strstr($_GET['filename'], 'offers') )
)
{
	echo "success\n";
	echo "exchange.parallels1c - offers.xml skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/exchange_parallel1c_exchange_admin.php?lang=ru";
}
elseif
(
	$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["DISALLOW_PRICES_XML_STEP"] == "Y"
	&&
	( $_GET['mode'] == 'import' && strstr($_GET['filename'], 'prices') )
)
{
	echo "success\n";
	echo "exchange.parallels1c - prices.xml skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/exchange_parallel1c_exchange_admin.php?lang=ru";
}
elseif
(
	$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["DISALLOW_RESTS_XML_STEP"] == "Y"
	&&
	( $_GET['mode'] == 'import' && strstr($_GET['filename'], 'rests') )
)
{
	echo "success\n";
	echo "exchange.parallels1c - rests.xml skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/exchange_parallel1c_exchange_admin.php?lang=ru";
}
elseif (
	...
	&&
	(
		$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["DISALLOW_DEACTIVATE_STEP"] == "Y"
		||
		$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["DISALLOW_IMPORT_XML_STEP"] == "Y"
		||
		$arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["IMPORT_XML_NEW_ONLY"] == "Y"
	)
)
{
	echo "success\n";
	echo "exchange.parallels1c - deactivation skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/exchange_parallel1c_exchange_admin.php?lang=ru";
}
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	if (!isset($arParams['IBLOCK_CACHE_MODE']))
		$arParams['IBLOCK_CACHE_MODE'] = CExchangeParallelsIBlockCMLImport::IBLOCK_CACHE_NORMAL;
	if (!in_array($arParams['IBLOCK_CACHE_MODE'], CExchangeParallelsIBlockCMLImport::getIblockCacheModeList(false)))
		$arParams['IBLOCK_CACHE_MODE'] = CExchangeParallelsIBlockCMLImport::IBLOCK_CACHE_NORMAL;
	...
	$importParameters = [
		...
		"table_name" => $table_name, // Exchange
		"iblock_cache_mode" => $arParams["IBLOCK_CACHE_MODE"]
	];

	// Exchange++
	if ( !isset( $NS["EXCHANGE_PARALLEL1C_BEFORE_STEP"] ) )
	{
		$NS["EXCHANGE_PARALLEL1C_BEFORE_STEP"] = true;
	}

	if ( $NS["EXCHANGE_PARALLEL1C_BEFORE_STEP"] )
	{
		//$strError = "";
		if (\Bitrix\Main\Config\Option::get("exchange.parallel1c", "copy_exchange_files") == "Y")
		{
			if (!isset($NS["EXCHANGE_PARALLEL1C_FILE_COPIED"]))
			{
				$dir_to = $_SERVER["DOCUMENT_ROOT"] . "/upload/1c_catalog_copy_exchange_parallel1c";
				...
				CopyDirFiles($ABS_FILE_NAME,
					$dir_to ."/". $arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["CODE"] . "/" . $arFileInfo["basename"]
				);

				$NS["EXCHANGE_PARALLEL1C_FILE_COPIED"] = true;
			}
		}
		...
		)
		{
			...
			$arRes = \Exchange\Parallel1c\Import::clearFileNewOnly($ABS_FILE_NAME, $ibPrefix);
			...
			if (\Bitrix\Main\Config\Option::get("exchange.parallel1c", "copy_exchange_files") == "Y")
			{
				...
				CopyDirFiles($ABS_FILE_NAME,
					$dir_to. "/" . $arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["CODE"] . "/" . $arFileInfo["filename"] . "_save_after_clear." . $arFileInfo["extension"]);
			}

			if ($arRes["message"])
			{
				$strMessage = "exchange.parallel1c: ".$arRes["message"];
			}

			if ($arRes["error_message"])
			{
				$strError = "exchange.parallel1c: ".$arRes["error_message"];
			}
		}
		...
		{
			$strMessage = "exchange.parallel1c: step passed";
		}

		$NS["EXCHANGE_PARALLEL1C_BEFORE_STEP"] = false;
	}
	elseif ($NS["STEP"] < 1)
	// Exchange--
	//if ($NS["STEP"] < 1)
	{
		...
		$obXMLFile = new CIBlockXMLFile( $table_name ); // Exchange
	...
	elseif ($NS["STEP"] == 4)
	{
		$obCatalog = new CExchangeParallelsIBlockCMLImport;
...
elseif ($_GET["mode"]=="deactivate")
{
	if ($_GET["timestamp"] > 0)
	{
		...
		while ($arField = $rsImportedIBlocks->fetch())
		{
			...
			$arAllowIblockKeys = $arParams["EXCHANGE_PARALLEL1C_SETTINGS"]["ALLOW_DEACTIVATE_IBLOCKS_ARRAY_KEYS"];
