...
if (!\Bitrix\Main\Loader::includeModule("data.parallel1c") )
{
	...
	echo "Module data.parallel1c not installed";
	die();
};

$arDataParallelsSettings = \Data\Parallel1c\Tools::getSettingsByPath();
if (!$arDataParallelsSettings)
{
	$APPLICATION->RestartBuffer();
	echo "data.parallel1c path not found!";
	die();
};
// Data--
...
elseif($type=="catalog")
{
	// Data++
	if (
		$arDataParallelsSettings["COMPONENT_IMPORT_VER"] == "last"
			||
		$arDataParallelsSettings["COMPONENT_IMPORT_VER"] == "20.0.0"
	)
	{
		$APPLICATION->IncludeComponent("data:data.parallel1c.catalog.import.1c.20.0.0", "", Array(
	...
	if ($arDataParallelsSettings["COMPONENT_IMPORT_VER"] == "17.6.3")
	{
		$APPLICATION->IncludeComponent("data:data.parallel1c.catalog.import.1c.17.6.3", "", Array(
				"IBLOCK_TYPE" => COption::GetOptionString("catalog", "1C_IBLOCK_TYPE", "-"),
				"SITE_LIST" => array(COption::GetOptionString("catalog", "1C_SITE_LIST", "-")),
				"INTERVAL" => COption::GetOptionString("catalog", "1C_INTERVAL", "-"),
				"GROUP_PERMISSIONS" => explode(",", COption::GetOptionString("catalog", "1C_GROUP_PERMISSIONS", "1")),
