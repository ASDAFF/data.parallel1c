...
if (!\Bitrix\Main\Loader::includeModule("exchange.parallel1c") )
{
	...
	echo "Module exchange.parallel1c not installed";
	die();
};

$arExchangeParallelsSettings = \Exchange\Parallel1c\Tools::getSettingsByPath();
if (!$arExchangeParallelsSettings)
{
	$APPLICATION->RestartBuffer();
	echo "exchange.parallel1c path not found!";
	die();
};
// Exchange--
...
elseif($type=="catalog")
{
	// Exchange++
	if (
		$arExchangeParallelsSettings["COMPONENT_IMPORT_VER"] == "last"
			||
		$arExchangeParallelsSettings["COMPONENT_IMPORT_VER"] == "20.0.0"
	)
	{
		$APPLICATION->IncludeComponent("exchange:exchange.parallel1c.catalog.import.1c.20.0.0", "", Array(
	...
	if ($arExchangeParallelsSettings["COMPONENT_IMPORT_VER"] == "17.6.3")
	{
		$APPLICATION->IncludeComponent("exchange:exchange.parallel1c.catalog.import.1c.17.6.3", "", Array(
				"IBLOCK_TYPE" => COption::GetOptionString("catalog", "1C_IBLOCK_TYPE", "-"),
				"SITE_LIST" => array(COption::GetOptionString("catalog", "1C_SITE_LIST", "-")),
				"INTERVAL" => COption::GetOptionString("catalog", "1C_INTERVAL", "-"),
				"GROUP_PERMISSIONS" => explode(",", COption::GetOptionString("catalog", "1C_GROUP_PERMISSIONS", "1")),
