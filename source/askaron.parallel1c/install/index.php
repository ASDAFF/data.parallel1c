...
class askaron_parallel1c extends CModule
{
    ...
    public function DoInstall()
    {
        ...
        if (is_array($this->_1499546494) && !empty($this->_1499546494)) foreach ($this->_1499546494 as $_255431949) if (!IsModuleInstalled($_255431949)) $askaron_parallel1c_global_errors[] = GetMessage('ASKARON_PARALLEL1C1_NEED_MODULES', array('#MODULE#' => $_255431949));
        if (strlen($this->_965838297) > 0 && version_compare(SM_VERSION, $this->_965838297) < 0) {
            $askaron_parallel1c_global_errors[] = GetMessage('ASKARON_PARALLEL1C1_NEED_RIGHT_VER', array('#NEED#' => $this->_965838297));
        }
        ...
        if (count($askaron_parallel1c_global_errors) == 0) {
            ...
            $_432070089 = $_969659624->isTableExists('b_askaron_parallel1c_exchange');
        ...
        $APPLICATION->IncludeAdminFile(GetMessage('ASKARON_PARALLEL1C1_INSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'] . '/' . $this->_1159779154 . '/modules/' . $this->MODULE_ID . '/install/step.php');
        return true;
    }
    ...
    public function DoUninstall()
    {
        ...
        if ($_388519351 >= 'W') {
            $step = IntVal($step);
            if ($step < 2) {
                $APPLICATION->IncludeAdminFile(GetMessage('ASKARON_PARALLEL1C1_UNINSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'] . '/' . $this->_1159779154 . '/modules/' . $this->MODULE_ID . '/install/unstep1.php');
            } elseif ($step == 2) {
                if ($_REQUEST['savedata'] != 'Y') {
                    \Bitrix\Main\Loader::includeModule($this->MODULE_ID);
                    $_1549870444 = \Askaron\Parallel1c\ExchangeTable::getList(array('select' => array('ID'),));
                    if ($_1451062140 = $_1549870444->fetch()) {
                        \Askaron\Parallel1c\ExchangeTable::delete($_1451062140['ID']);
                    }
                    $this->UnInstallDB();
                }
                $this->UnInstallFiles();
                UnRegisterModule('askaron.parallel1c');
                $APPLICATION->IncludeAdminFile(GetMessage('ASKARON_PARALLEL1C1_UNINSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'] . '/' . $this->_1159779154 . '/modules/' . $this->MODULE_ID . '/install/unstep2.php');
                return true;
            }
        }
    }
    ...
    function InstallFiles($_1735337710 = array())
    {
        ...
        CopyDirFiles($_SERVER['DOCUMENT_ROOT'] . '/' . $this->_1159779154 . '/modules/' . $this->MODULE_ID . '/install/components/askaron/', $_SERVER['DOCUMENT_ROOT'] . '/bitrix/components/askaron/', true, true);
    ...
    function InstallDB()
    {
        ...
        if (!$DB->Query("SELECT 'x' FROM b_askaron_parallel1c_exchange", true)) $_72159314 = "Y"; else $_72159314 = "N";
        $_730090081 = false;
        if ($_72159314 == "Y") {
