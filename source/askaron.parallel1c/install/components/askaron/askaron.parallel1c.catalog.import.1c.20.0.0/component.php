...
// Askaron++
//CIBlockCMLImport replace to CAskaronParallelsIBlockCMLImport
//$arParams["ASKARON_PARALLEL1C_SETTINGS_ID"] = intval($arParams["ASKARON_PARALLEL1C_SETTINGS_ID"]);

\Bitrix\Main\Loader::includeModule("askaron.parallel1c");
$arParams["ASKARON_PARALLEL1C_SETTINGS"] = \Askaron\Parallel1c\Tools::getSettingsByPath();

$table_name = "b_xml_tree_askaron_parallel1c_".$arParams["ASKARON_PARALLEL1C_SETTINGS"]["CODE"];
$catalog_name = "1c_catalog_askaron_parallel1c_".$arParams["ASKARON_PARALLEL1C_SETTINGS"]["CODE"];
// Askaron--
...
// Askaron++
elseif ( !$arParams["ASKARON_PARALLEL1C_SETTINGS"] )
{
	echo "failure\n";
	echo "askaron.parallels1c - settings not found\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/askaron_parallel1c_exchange_admin.php?lang=ru";
}
elseif
(
	$arParams["ASKARON_PARALLEL1C_SETTINGS"]["DISALLOW_IMPORT_XML_STEP"] == "Y"
	&&
	( $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import') )
)
{
	echo "success\n";
	echo "askaron.parallels1c - import.xml skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/askaron_parallel1c_exchange_admin.php?lang=ru";
}
elseif
(
	$arParams["ASKARON_PARALLEL1C_SETTINGS"]["DISALLOW_OFFERS_XML_STEP"] == "Y"
	&&
	( $_GET['mode'] == 'import' && strstr($_GET['filename'], 'offers') )
)
{
	echo "success\n";
	echo "askaron.parallels1c - offers.xml skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/askaron_parallel1c_exchange_admin.php?lang=ru";
}
elseif
(
	$arParams["ASKARON_PARALLEL1C_SETTINGS"]["DISALLOW_PRICES_XML_STEP"] == "Y"
	&&
	( $_GET['mode'] == 'import' && strstr($_GET['filename'], 'prices') )
)
{
	echo "success\n";
	echo "askaron.parallels1c - prices.xml skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/askaron_parallel1c_exchange_admin.php?lang=ru";
}
elseif
(
	$arParams["ASKARON_PARALLEL1C_SETTINGS"]["DISALLOW_RESTS_XML_STEP"] == "Y"
	&&
	( $_GET['mode'] == 'import' && strstr($_GET['filename'], 'rests') )
)
{
	echo "success\n";
	echo "askaron.parallels1c - rests.xml skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/askaron_parallel1c_exchange_admin.php?lang=ru";
}
elseif (
	...
	&&
	(
		$arParams["ASKARON_PARALLEL1C_SETTINGS"]["DISALLOW_DEACTIVATE_STEP"] == "Y"
		||
		$arParams["ASKARON_PARALLEL1C_SETTINGS"]["DISALLOW_IMPORT_XML_STEP"] == "Y"
		||
		$arParams["ASKARON_PARALLEL1C_SETTINGS"]["IMPORT_XML_NEW_ONLY"] == "Y"
	)
)
{
	echo "success\n";
	echo "askaron.parallels1c - deactivation skipped\n";
	echo "http://".$_SERVER["SERVER_NAME"]."/bitrix/admin/askaron_parallel1c_exchange_admin.php?lang=ru";
}
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	if (!isset($arParams['IBLOCK_CACHE_MODE']))
		$arParams['IBLOCK_CACHE_MODE'] = CAskaronParallelsIBlockCMLImport::IBLOCK_CACHE_NORMAL;
	if (!in_array($arParams['IBLOCK_CACHE_MODE'], CAskaronParallelsIBlockCMLImport::getIblockCacheModeList(false)))
		$arParams['IBLOCK_CACHE_MODE'] = CAskaronParallelsIBlockCMLImport::IBLOCK_CACHE_NORMAL;
	...
	$importParameters = [
		...
		"table_name" => $table_name, // Askaron
		"iblock_cache_mode" => $arParams["IBLOCK_CACHE_MODE"]
	];

	// Askaron++
	if ( !isset( $NS["ASKARON_PARALLEL1C_BEFORE_STEP"] ) )
	{
		$NS["ASKARON_PARALLEL1C_BEFORE_STEP"] = true;
	}

	if ( $NS["ASKARON_PARALLEL1C_BEFORE_STEP"] )
	{
		//$strError = "";
		if (\Bitrix\Main\Config\Option::get("askaron.parallel1c", "copy_exchange_files") == "Y")
		{
			if (!isset($NS["ASKARON_PARALLEL1C_FILE_COPIED"]))
			{
				$dir_to = $_SERVER["DOCUMENT_ROOT"] . "/upload/1c_catalog_copy_askaron_parallel1c";
				...
				CopyDirFiles($ABS_FILE_NAME,
					$dir_to ."/". $arParams["ASKARON_PARALLEL1C_SETTINGS"]["CODE"] . "/" . $arFileInfo["basename"]
				);

				$NS["ASKARON_PARALLEL1C_FILE_COPIED"] = true;
			}
		}
		...
		)
		{
			...
			$arRes = \Askaron\Parallel1c\Import::clearFileNewOnly($ABS_FILE_NAME, $ibPrefix);
			...
			if (\Bitrix\Main\Config\Option::get("askaron.parallel1c", "copy_exchange_files") == "Y")
			{
				...
				CopyDirFiles($ABS_FILE_NAME,
					$dir_to. "/" . $arParams["ASKARON_PARALLEL1C_SETTINGS"]["CODE"] . "/" . $arFileInfo["filename"] . "_save_after_clear." . $arFileInfo["extension"]);
			}

			if ($arRes["message"])
			{
				$strMessage = "askaron.parallel1c: ".$arRes["message"];
			}

			if ($arRes["error_message"])
			{
				$strError = "askaron.parallel1c: ".$arRes["error_message"];
			}
		}
		...
		{
			$strMessage = "askaron.parallel1c: step passed";
		}

		$NS["ASKARON_PARALLEL1C_BEFORE_STEP"] = false;
	}
	elseif ($NS["STEP"] < 1)
	// Askaron--
	//if ($NS["STEP"] < 1)
	{
		...
		$obXMLFile = new CIBlockXMLFile( $table_name ); // Askaron
	...
	elseif ($NS["STEP"] == 4)
	{
		$obCatalog = new CAskaronParallelsIBlockCMLImport;
...
elseif ($_GET["mode"]=="deactivate")
{
	if ($_GET["timestamp"] > 0)
	{
		...
		while ($arField = $rsImportedIBlocks->fetch())
		{
			...
			$arAllowIblockKeys = $arParams["ASKARON_PARALLEL1C_SETTINGS"]["ALLOW_DEACTIVATE_IBLOCKS_ARRAY_KEYS"];
